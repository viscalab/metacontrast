...
```{r}
SZ_pooled <- pool.groups(n1 = 14,   # sample size group 1
...
```{r}
oDonnell_2006_es <- esc_mean_sd(grp1m = 1.74, grp2m = SZ_pooled$Mpooled,  # group mean
            grp1sd = 0.45, grp2sd = SZ_pooled$SDpooled, # group sd
            grp1n = 40, grp2n = SZ_pooled$Npooled, 
            es.type = "g")
oDonnell_2006_es
```
...
oDonnell_2006$method <- "staircase" # adaptative, mc, ... (Do they use a special method to display contrast?) 
...
oDonnell_2006$lapses <- "FALSE"  #In lapses, we can include whether they control for lapses (TRUE) or not (FALSE).
oDonnell_2006$calculation_statistic <- "m and sd"  #"t-test", "m and sd"
oDonnell_2006$calculation_data <- "reported, pooling sd" #"scrapping", "reported", "raw"
oDonnell_2006$calculation_certainty <- "unsure" #scale is "sure", "unsure", "very unsure"
...
I believe the f-statistic is not entirely correct.It compares ctrl vs scz, but not sure if this is only for static, or also for motion.
